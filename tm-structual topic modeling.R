...
blogs <- read_csv(file = "D:/대학원/논문/topic model/poliblogs2008.csv", col_names = TRUE, locale = locale('ko',encoding='utf-8'))
...
names(blogs)[1] <- c("num_id")


# default parameters
news_processed <- textProcessor(
  blogs$documents, metadata = blogs,
...
news_processed_out <- prepDocuments(
  news_processed$documents, 
  news_processed$vocab, 
  news_processed$meta, lower.thresh = 10)
...
docs <- news_processed_out$documents
vocab <- news_processed_out$vocab
meta <- news_processed_out$meta

set.seed(1029)
First_STM <- stm(documents = docs, 
                 vocab = vocab, 
                 K = 15,
...
# 두 가지 설명 변수를 사용하여 주제 출현을 제어합니다. rating은 문서의 평가나 등급을 나타내며, day는 문서가 작성된 날짜를 나타냅니다. 이렇게 함으로써 주제가 문서의 등급과 날짜에 따라 어떻게 분포하는지를 고려할 수 있게 됩니다.
...
# 1
findingk <- searchK(documents = docs, 
                    vocab = vocab, 
                    K = c(10:30), 
                    prevalence = ~ rating + s(day), 
                    data = meta, verbose = FALSE) # spend long time
