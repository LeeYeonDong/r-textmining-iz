...
# 날짜 변수를 숫자로 변환
cp_df_raw$날짜 <- as.numeric(cp_df_raw$날짜)

# 필요한 열 추출
cp_df_pos_s <- cp_df_raw %>%
  select(category1, 날짜, 본문)

# 텍스트 데이터 전처리
cp_df_pos_s$본문 <- str_replace_all(cp_df_pos_s$본문, "[^가-힣a-zA-Z0-9.,!?\\s]", " ")


# 텍스트 처리
cp_processed <- textProcessor(
  documents = cp_df_pos_s$본문, 
  metadata = cp_df_pos_s,
  lowercase = TRUE,             
...
cp_processed_out <- prepDocuments(
  ...
  cp_processed$vocab, 
  cp_processed$meta, 
  lower.thresh = 20 # 최소 20회 이상 등장하는 단어만 사용
)
