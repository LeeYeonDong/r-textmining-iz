...
reviews <- read_csv(file = "C:/대학원/논문/소논문/뉴스_sports.news_NC.csv", col_names = TRUE, locale=locale('ko',encoding='euc-kr'))

names(reviews) <- c("언론사_NC","제목_NC","날짜_NC","링크_NC","좋아_NC","훈훈_NC","슬퍼_NC","화나_NC","후속_NC")

reviews <- reviews[!is.na(reviews$날짜_NC),]

reviews$ID_NC <- c(1:nrow(reviews))

reviews$제목_NC <- gsub("포토","",reviews$제목_NC)
reviews$제목_NC <- gsub("오늘","",reviews$제목_NC)
reviews$제목_NC <- gsub("경기","",reviews$제목_NC)
reviews$제목_NC <- gsub("사진","",reviews$제목_NC)
reviews$제목_NC <- gsub("스포츠","",reviews$제목_NC)
reviews$제목_NC <- gsub("종합","",reviews$제목_NC)
reviews$제목_NC <- gsub("다시","",reviews$제목_NC)
reviews$제목_NC <- gsub("치어리더","",reviews$제목_NC)
reviews$제목_NC <- gsub("\u7f8e","",reviews$제목_NC)
reviews$제목_NC <- gsub("\u65e5","",reviews$제목_NC)
...
reviews_tokens <- reviews %>% 
  dplyr::select(ID_NC, 제목_NC) %>% # MASS랑 충돌
...
reviews_tokens %>% 
  ...
  mutate(n_tokens_binned = cut(n_tokens, breaks = c(1,seq(0,40,5),Inf))) %>% 
...
# 신경망을 구축 할 때 총 5 개 이상의 빈도를 가진 단어 만 사용합니다. 현재 컬렉션은 37.520 개의 고유 한 단어로 내려갑니다.
reviews_tokens %>% 
  group_by(word) %>% 
