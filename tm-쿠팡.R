...
for(j in 1:length(category1)){
for(i in 1:length(cp_df)){
  tryCatch({
    # SimplePos22 함수 적용
    cp_df_tmp <- cp_df[[i]] %>%
      filter(category1 == category1[j]) %>% 
      select(본문)
  ...
  cp_df_pos_tmp <- sapply(cp_df_tmp$본문, function(x) {
      SimplePos22(x)
    }) %>%
      melt() %>%
      as_tibble() %>%
      ...
      filter(str_detect(value, "/NC") | str_detect(value, "/PA") | str_detect(value, "/PV")) %>%
      mutate(value = str_match(value, '([가-힣]+)')[, 2]) %>%
      mutate(value = sapply(value, remove_josa, patterns = josa_patterns)) %>%
      filter(str_length(value) >= 2)
    
    cp_df_pos <- bind_rows(cp_df_pos, cp_df_pos_tmp)
    cp_df_pos$category1 <- category1[j]
...
# 결과를 엑셀 파일로 저장
library(writexl)
write_xlsx(cp_df_pos, "D:/대학원/상담/커뮤니케이션학과/쇼핑몰/cp_df_pos.xlsx")

cp_df_pos <- read_excel("D:/대학원/상담/커뮤니케이션학과/쇼핑몰/cp_df_pos.xlsx")
...
wordcloud2(token_count30_사, minRotation = 0, maxRotation = 0, color = "blue") 

wordcloud2(token_count30_생, minRotation = 0, maxRotation = 0, color = "blue") 

# tf-idf
cp_df_pos_idf_사 <- cp_df_pos %>% 
  filter(category1 == category1[1]) %>% 
  count(L1, value) %>% 
  filter(str_count(value) > 1) %>% 
  bind_tf_idf(term = value, document = L1, n = n) %>% 
  arrange(-tf_idf)

cp_df_pos_idf_생 <- cp_df_pos %>% 
...
write_xlsx(cp_df_pos_idf_사, "D:/대학원/상담/커뮤니케이션학과/쇼핑몰/cp_df_pos_idf_사치품.xlsx")

write_xlsx(cp_df_pos_idf_생, "D:/대학원/상담/커뮤니케이션학과/쇼핑몰/cp_df_pos_idf_생필품.xlsx")
