...
n4g_KM %>% ggsurvplot(conf.int = TRUE, surv.median.line = "hv", pval = TRUE, pval.method = TRUE)

# Cox Proportional Hazard model
n4g_cox1 <- coxph(
  Surv(time = n4g$댓글,
       event = n4g$관심도over) ~ n4g$이름, data = n4g)

n4g_cox1 %>% summary()

n4g_cox2 <- coxph(
  Surv(time = n4g$댓글,
       event = n4g$관심도over) ~ n4g$이름 + n4g$날짜, data = n4g)
...
ggcoxdiagnostics(n4g_cox1, type = "martingale", linear.predictions = TRUE)
