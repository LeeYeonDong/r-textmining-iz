...
# data
news.naver_야구 <- read_csv(file = "C:/대학원/논문/소논문/뉴스_news.naver_야구.csv", col_names = TRUE, locale=locale('ko',encoding='euc-kr'))
news.naver_야구 %>% str()
news.naver_야구 <- news.naver_야구 %>%  select("언론사_야구","제목_야구","날짜_야구","링크_야구","좋아_야구","훈훈_야구","슬퍼_야구","화나_야구","후속_야구")

sports.news_야구 <- read_csv(file = "C:/대학원/논문/소논문/뉴스_sports.news_야구.csv", col_names = TRUE, locale=locale('ko',encoding='euc-kr'))
sports.news_야구 %>% str()
