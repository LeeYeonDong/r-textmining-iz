...
umap_params = [
    ...
    for n, c, d, m in product([15, 30], [5, 10], [0.0, 0.1, 0.2], ['euclidean', 'cosine'])
]
hdbscan_params = [
    {"min_cluster_size": s, "min_samples": ms, "metric": m, "prediction_data": True}
    for s, ms, m in product([5, 10], [None, 5, 10], ['euclidean', 'manhattan'])
]
...
results_df.to_csv('D:/대학원/논문/소논문/부동산_토픽모델링/gridsearch_umap_hdbscan.csv', index=False, encoding='cp949')
