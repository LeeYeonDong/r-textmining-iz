...
raw3_before_dtm <- raw3_token_df %>% 
                    group_by(id) %>% 
                    count(단어, sort = TRUE)

raw3_dtm <- cast_dtm(raw3_before_dtm, document = id, term = 단어, value = n)
class(raw3_dtm)
as.matrix(raw3_dtm[1:10, 1:10]) %>% view()
...
FindTopicsNumber_plot(raw3_lda_n)


# run LDA
set.seed(1029)
raw3_lda <- LDA(raw3_dtm, k = 8, method = "Gibbs", control = list(iter = 500, alpha = 1, delta = 0.1))
...
raw3_topics <- tidy(raw3_lda, matrix = "beta")
# beta : 토픽에 단어가 들어갈 확률
# gamma : 문서가 각 토픽에 등장할 확률

raw3_top_terms <- raw3_topics %>% 
  group_by(topic) %>%
  top_n(20, beta) %>%
  arrange(topic, -beta)
...
# 문서를 토픽별로 분류하기
raw3_topic <- tidy(raw3_lda, matrix = "gamma")
# gamma는 문서가 각 토픽에 등장할 확률
raw3_topic %>% filter(topic == c(3))
raw3_token_df %>% filter(id %in% c(7238, 7184, 7355, 7394, 6840)) %>% view()
# lec 02.25.


# 문서별로 확률이 가장 높은 토픽 추출
raw3_class <- raw3_topic %>% 
  group_by(document) %>% 
  slice_max(gamma, n = 1)
...
##원문에 확률이 가장 높은 번호 부여
#integer로 변환
raw3_class$document <- as.integer(raw3_class$document)

raw3_class_topic <- left_join(raw3_token_df, raw3_class, by = c("id" = "document")) #vlookup

# 토픽별 단어 수 살펴보기
raw3_class_topic_count <- count(raw3_class_topic, topic)

# 토픽별 주요 단어 목록 만들기
raw3_terms <- raw3_top_terms %>% 
  group_by(topic) %>%
  slice_max(beta, n = 5, with_ties = FALSE) %>% 
  summarise(term = paste(term, collapse = ", "))

# 문서 빈도에 주요단어 결합
raw3_topic_count_word <- raw3_class_topic_count %>% 
  left_join(raw3_terms, by = "topic")

raw3_topic_count_word$topic <- paste("topic", raw3_topic_count_word$topic)
raw3_topic_count_word$n <- raw3_topic_count_word$n %>% as.character()

# 토픽별 문서 수와 주요 단어로 막대 그래프 만들기
raw3_topic_count_word %>% 
...
serVis(topicmodels2LDAvis(raw3_lda))
raw3_lda %>% topicmodels2LDAvis() %>% serVis()
...
raw3
raw3$일자 <- str_sub(raw3$일자,1,6)

# get mean topic proportions per decade = dynamic topic modeling
library(RColorBrewer)

aggregate(posterior(raw3_lda)$topics, by = list(month = raw3$일자), mean) %>% 
