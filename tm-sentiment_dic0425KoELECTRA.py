...
# sentiment와 score를 포함한 결과를 DataFrame에 저장
df['result'] = df['제목'].apply(lambda x: nlp(x)[0] if x.strip() != "" else {'label': 'Neutral', 'score': None})

# label과 score를 별도의 열로 저장
df['sentiment'] = df['result'].apply(lambda x: x['label'])
df['score'] = df['result'].apply(lambda x: x['score'])

# result 열이 더 이상 필요 없으면 삭제
...
file_path = "D:\\대학원\\논문\\소논문\\부동산_감정사전\\re_df0425.csv"
df.to_csv(file_path, index=False, encoding='utf-8-sig') # cp949 utf-8 깨짐

# 'average_score'로 Pos Neg Neu 나누기 => "sen_pol" 변수
...
##
df = pd.read_csv("D:/대학원/논문/소논문/부동산_감정사전/re_df0425.csv", encoding='utf-8-sig')

df['sen_pol'] = df['sentiment'].apply(lambda x: 1 if x == 'LABEL_1' else 0)
...
# 지정된 경로에 DataFrame을 CSV 파일로 저장
file_path = "D:\\대학원\\논문\\소논문\\부동산_감정사전\\dict0425.csv"
df_sentiment.to_csv(file_path, index=False,  encoding='utf-8-sig')
